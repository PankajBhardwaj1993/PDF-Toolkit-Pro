...
const SinglePdfPreview: React.FC<{ file: File; onPageCountLoaded: (count: number) => void }> = ({ file, onPageCountLoaded }) => {
  ...
  useEffect(() => {
    ...
    const renderFirstPage = async () => {
      try {
        ...
        if (!pdfjs) {
          // Retry in 300ms if pdfjs is still loading
          setTimeout(renderFirstPage, 300);
          return;
        }
        ...
        const scale = 110 / rawViewport.width; // Fixed thumbnail width of 110px
  ...
  if (error) {
    return (
      <div className="w-[110px] aspect-[1/1.4] bg-slate-100 dark:bg-zinc-900 rounded-lg flex flex-col items-center justify-center p-2 text-center border border-slate-200 dark:border-zinc-800">
        ...
        <span className="text-[8px] text-rose-500 mt-1">No Preview</span>
      </div>
    );
  }

  return (
    <div className="relative shadow border border-slate-200 dark:border-zinc-800 rounded-lg overflow-hidden bg-white dark:bg-zinc-900 w-[110px] aspect-[1/1.4] flex items-center justify-center flex-shrink-0">
      {loading ? (
