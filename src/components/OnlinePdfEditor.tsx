...
import { 
  ...
  AlignLeft, AlignCenter, AlignRight, Globe, Languages, Search
} from 'lucide-react';
import { PDFDocument, rgb, StandardFonts, degrees } from 'pdf-lib';
import { SUPPORTED_LANGUAGES_LIST, detectScriptAndDirection, SupportedLanguage } from '../utils/languageConfig';
import { getAppropriateUnicodeFont, needsMultiLanguageFont } from '../utils/unicodeFontManager';
...
export function detectFontFamilyAndSize(item: any, style: any = {}): DetectedFontStyles {
  ...
  // 1. In standard PDF text matrix [a, b, c, d, e, f]:
  // [a, b] is horizontal vector (including horizontal scaling), [c, d] is vertical vector (font size in points)
  if (item && item.transform && Array.isArray(item.transform) && item.transform.length >= 4) {
    ...
    const vertSize = Math.hypot(skewX, scaleY) || Math.abs(scaleY);
    const horizSize = Math.hypot(scaleX, skewY) || Math.abs(scaleX);
    
    // In PDF standard, font size in points corresponds to the vertical glyph baseline scale
    const computedSize = vertSize > 0 ? vertSize : horizSize;
    if (computedSize > 0) {
      fontSize = Math.round(computedSize * 100) / 100;
    }
  }

  // 2. Fall back to item.height if transform did not give a valid size
  if ((!fontSize || fontSize <= 0) && item && typeof item.height === 'number' && item.height > 0) {
    fontSize = Math.round(item.height * 100) / 100;
  }

  // 3. Fallback only if completely missing or non-positive (never override valid small font sizes like 5.5pt or 6pt)
  ...
  const bold = /bold|black|heavy|bd|-b\b|_b\b|medium|semibold|demi|700|800|900/i.test(fontLower) || 
               (style?.fontWeight && (String(style.fontWeight).toLowerCase().includes('bold') || Number(style.fontWeight) >= 600));
  const italic = /italic|oblique|slanted|it\b|-i\b|_i\b/i.test(fontLower) || 
                (style?.fontStyle && String(style.fontStyle).toLowerCase().includes('italic'));
...
async function getAppropriateFont(
  ...
  try {
    return await getAppropriateUnicodeFont(
      pdfDoc,
      fontFamily,
      isBold,
      isItalic,
    ...
    console.warn("getAppropriateUnicodeFont failed, using standard Helvetica font:", err);
...
function SinglePdfPage({
  ...
  // Script and RTL detection for active language
  const detectedLangInfo = detectScriptAndDirection(newTextValue);
  const activeLangConfig = SUPPORTED_LANGUAGES_LIST.find(
    l => l.code === (selectedLanguage && selectedLanguage !== 'auto' ? selectedLanguage : detectedLangInfo.languageCode)
  ) || SUPPORTED_LANGUAGES_LIST.find(l => l.code === 'en') || SUPPORTED_LANGUAGES_LIST[0];
  const isRtl = activeLangConfig.isRtl || detectedLangInfo.isRtl;

  const insertQuickChar = (char: string) => {
    const textarea = document.getElementById('pdf-text-editor-textarea') as HTMLTextAreaElement | null;
    if (textarea) {
      const start = textarea.selectionStart ?? newTextValue.length;
      const end = textarea.selectionEnd ?? newTextValue.length;
      const updated = newTextValue.substring(0, start) + char + newTextValue.substring(end);
      setNewTextValue(updated);
      setTimeout(() => {
        textarea.focus();
        textarea.setSelectionRange(start + char.length, start + char.length);
      }, 10);
    } else {
      setNewTextValue(newTextValue + char);
    }
  };
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-0.5 px-1 bg-slate-50 dark:bg-zinc-950 rounded-lg py-0.5">
                            <button 
                              type="button"
                              onClick={() => setEditingBlock({ ...editingBlock, fontSize: Math.max(3, Math.round((editingBlock.fontSize - 0.5) * 10) / 10) })}
                            ...
                            <span className="font-mono font-bold text-[10px] w-10 text-center text-slate-600 dark:text-zinc-400">
                              {Number(editingBlock.fontSize.toFixed(1))}pt
                            </span>
                            <button 
                              type="button"
                              onClick={() => setEditingBlock({ ...editingBlock, fontSize: Math.round((editingBlock.fontSize + 0.5) * 10) / 10 })}
                              className="p-1 hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-500 dark:text-zinc-400 rounded cursor-pointer transition-colors"
                        ...
                        {/* Language Script Badge & Quick Special Characters */}
                        <div className="flex items-center gap-1.5 mb-1.5 bg-slate-900/90 dark:bg-zinc-950 text-white rounded-lg px-2.5 py-1 shadow-lg text-[10px] max-w-[420px] overflow-x-auto flex-wrap">
                          <div className="flex items-center gap-1 font-semibold text-amber-300 pr-1.5 border-r border-slate-700 whitespace-nowrap">
                            <span>{activeLangConfig.flag}</span>
                            <span>{activeLangConfig.nativeName}</span>
                            {isRtl && <span className="bg-amber-500/20 text-amber-300 px-1 rounded text-[9px] font-bold">RTL</span>}
                          </div>
                          {activeLangConfig.quickChars && activeLangConfig.quickChars.length > 0 && (
                            <div className="flex items-center gap-1 overflow-x-auto py-0.5">
                              {activeLangConfig.quickChars.slice(0, 15).map((ch, idx) => (
                                <button
                                  key={idx}
                                  type="button"
                                  onClick={() => insertQuickChar(ch)}
                                  className="px-1.5 py-0.5 bg-slate-800 hover:bg-blue-600 rounded text-[11px] font-medium text-slate-200 hover:text-white transition-colors cursor-pointer"
                                  title={`Insert ${ch}`}
                                >
                                  {ch}
                                </button>
                          ...
                          return (
                            <textarea
                              ...
                              style={{
                                ...
                                textAlign: editingBlock.align || (isRtl ? 'right' : 'left'),
                                fontSize: `${visualFontSize}px`,
                                fontFamily: editingBlock.fontFamily || activeLangConfig.fontFamily,
...
export default function OnlinePdfEditor({ onAddRecentFile, user }: OnlinePdfEditorProps) {
    ...
    try {
      ...
      for (const pIdx of targetIndices) {
        if (pIdx >= 0 && pIdx < pagesList.length) {
          ...
          page.drawText(sanitizeForWinAnsi(watermarkText || 'CONFIDENTIAL'), {
            x: pageW * 0.15,
            y: pageH * 0.45,
            size: 48,
            font,
        ...
        if (pNum >= 1 && pNum <= pagesList.length) {
          ...
          for (const ann of list) {
            ...
            if ((ann.type === 'draw' || ann.type === 'erase' || ann.type === 'line' || ann.type === 'connected_lines') && ann.points.length >= 2) {
              ...
              page.drawText(sanitizeForWinAnsi(stampText), {
                x: stampX + 12,
                y: stampY + 10,
                size: 11,
                font,
              ...
              const centerX = (ann.points[0].x / 100) * pageW;
              const centerY = pageH - (ann.points[0].y / 100) * pageH;
              // Draw Green circle badge matching SVG
              page.drawEllipse({
                x: centerX,
                y: centerY,
                xScale: 12,
              ...
              // Draw White vector Checkmark
              page.drawLine({
                start: { x: centerX - 5.5, y: centerY },
                end: { x: centerX - 1.5, y: centerY - 4.5 },
                thickness: 2.2,
                color: rgb(1, 1, 1),
              });
              page.drawLine({
                start: { x: centerX - 1.5, y: centerY - 4.5 },
                end: { x: centerX + 5.5, y: centerY + 4.5 },
                thickness: 2.2,
                color: rgb(1, 1, 1),
              });
              ...
              const centerX = (ann.points[0].x / 100) * pageW;
              const centerY = pageH - (ann.points[0].y / 100) * pageH;
              // Draw Red circle badge matching SVG
              page.drawEllipse({
                x: centerX,
                y: centerY,
                xScale: 12,
              ...
              // Draw White vector Cross
              page.drawLine({
                start: { x: centerX - 4.5, y: centerY - 4.5 },
                end: { x: centerX + 4.5, y: centerY + 4.5 },
                thickness: 2.2,
                color: rgb(1, 1, 1),
              });
              page.drawLine({
              ...
              page.drawText(sanitizeForWinAnsi(dateText), { x: ptX, y: ptY, size: 12, font, color: strokeRgb });
              ...
              page.drawText(sanitizeForWinAnsi(ann.text || 'Sticky Note'), { x: ptX + 5, y: ptY + 40, size: 10, font, color: rgb(0.27, 0.1, 0.01) });
  ...
  const handleApplyTextEdit = async (options?: { customReplacementText?: string }) => {
    ...
    try {
      ...
      // Preserve the exact PDF coordinates of the text block to prevent table alignment shifting
      let finalPdfX = blockToUse.pdfX;
      let finalPdfW = blockToUse.pdfW || (blockToUse.w / 100) * origPageW || 150;

      // Available line width for text reflow/wrapping: respect block boundary or page margin
      const margin = 28.3;
      const maxLineWidth = Math.max(
        blockToUse.pdfW || 0,
        origPageW - finalPdfX - margin
      );

      // Perform font-metrics word wrapping preserving exact baseFontSize
      let wrappedLines: string[];
      if (targetText.includes('\n')) {
        wrappedLines = wrapTextForPdf(targetText, maxLineWidth, baseFontSize, font);
      } else {
        let textMeasuredWidth = 0;
        try {
          if (font) {
            textMeasuredWidth = font.widthOfTextAtSize(targetText, baseFontSize);
          }
        } catch (e) {
          textMeasuredWidth = targetText.length * baseFontSize * 0.6;
        }

        // If text is longer than available line width, wrap/reflow cleanly while strictly maintaining baseFontSize
        if (textMeasuredWidth > maxLineWidth && maxLineWidth > 40) {
          wrappedLines = wrapTextForPdf(targetText, maxLineWidth, baseFontSize, font);
        } else {
      ...
      const descenderOffset = baseFontSize * 0.20;
      const ascenderOffset = baseFontSize * 0.80;
      const lineSpacing = baseFontSize * 1.20;
      ...
      // Erase original text rectangle (Only the exact original text bounds, preserving table border lines and grid cells)
      if (!blockToUse.isNew && (blockToUse.pdfW > 0 || blockToUse.text || blockToUse.originalText)) {
        const origFontSize = blockToUse.fontSize || 9;
        
        // Font glyph vertical offsets strictly constrained to font metrics to prevent erasing horizontal cell borders:
        // Ascender (capitals, numbers, standard glyphs): 0.80 * fontSize above baseline
        // Descender ('g', 'j', 'p', 'q', 'y'): 0.20 * fontSize below baseline
        const origDescenderOffset = origFontSize * 0.20;
        const origAscenderOffset = origFontSize * 0.80;
        ...
        // Exact text bounding width without bleeding into adjacent table lines or borders
        const maxTextW = Math.max(
          origTextMeasuredWidth,
          blockToUse.initialPdfW || 0,
          blockToUse.pdfW || 0,
          newTextMeasuredWidth
        );

        // Precise boundary: start exactly at pdfX without negative offsets, and cap tightly without +5 over-erasure
        const eraseWidth = Math.min(origPageW - blockToUse.pdfX, maxTextW + 0.1);
        const origRectX = Math.max(0, blockToUse.pdfX);

        const origLines = origTextStr.includes('\n') ? origTextStr.split('\n') : [origTextStr];
        const origLineSpacing = origFontSize * 1.15;
        const numLines = Math.max(origLines.length, wrappedLines.length);
        const origRectY = blockToUse.pdfY - ((numLines - 1) * origLineSpacing) - origDescenderOffset;
        const origRectH = ((numLines - 1) * origLineSpacing) + origAscenderOffset + origDescenderOffset;
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {!pdfSource ? (
        ...
        <div className="bg-slate-100 dark:bg-zinc-950 rounded-3xl border border-slate-200 dark:border-zinc-850 overflow-hidden shadow-2xl flex flex-col">
          ...
          <div className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-850 px-6 py-4 flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 bg-blue-50 dark:bg-blue-950/60 hover:bg-blue-100 dark:hover:bg-blue-900/60 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 font-semibold px-3 py-2 rounded-xl text-xs cursor-pointer shadow-sm transition-all"
                title="Select Language & Font Support (29 Languages: Arabic, Hindi, Hebrew, Thai, CJK, etc.)"
              >
                <Globe className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />
                <span className="font-medium">
                  {selectedLanguage === 'auto'
                    ? '🌐 Auto (29 Langs)'
                    : `${SUPPORTED_LANGUAGES_LIST.find(l => l.code === selectedLanguage)?.flag || '🌐'} ${SUPPORTED_LANGUAGES_LIST.find(l => l.code === selectedLanguage)?.nativeName || selectedLanguage}`}
                </span>
          ...
          <div className="bg-slate-100/90 dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 px-4 py-2 flex items-center justify-start gap-1 sm:gap-2 overflow-x-auto select-none no-scrollbar shadow-inner">
            ...
            {/* 8. Multi-Language / 29 Languages */}
            <button
              onClick={() => setIsLanguageModalOpen(true)}
              className="flex flex-col items-center justify-center min-w-[62px] px-2.5 py-1 rounded-xl cursor-pointer transition-all text-slate-700 dark:text-zinc-300 hover:bg-slate-200/70 dark:hover:bg-zinc-800 border border-slate-200/60 dark:border-zinc-700/60 bg-white/50 dark:bg-zinc-800/50"
              title="Change Editor Language & Unicode Script (29 Languages Supported)"
            >
              ...
              <span className="text-[11px] font-semibold mt-0.5 whitespace-nowrap">
                {selectedLanguage === 'auto' ? 'Language' : (SUPPORTED_LANGUAGES_LIST.find(l => l.code === selectedLanguage)?.flag || 'Lang')}
              </span>
            </button>
      ...
      {/* 29 Supported Languages Selection Modal */}
      {isLanguageModalOpen && (
        <div 
        ...
        >
          <div 
            className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden animate-scale-up"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="p-5 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-sm">
                  <Languages className="h-5 w-5" />
                </div>
                <div>
                  <h3 className="font-bold text-base text-slate-900 dark:text-zinc-100 flex items-center gap-2">
                    <span>PDF Editor Languages & Font Scripts</span>
                    <span className="text-[10px] bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 font-bold px-2 py-0.5 rounded-full border border-indigo-200 dark:border-indigo-800">
                      29 Languages
                    </span>
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
                    High-quality Unicode rendering with font embedding & RTL support
                  </p>
                </div>
              </div>
              <button 
            ...
            {/* Search filter bar */}
            <div className="p-4 border-b border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/50 flex items-center gap-2">
              <div className="relative flex-1">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <input
                  type="text"
                  placeholder="Search languages (e.g. Hindi, Arabic, Japanese, Deutsch)..."
                  value={languageSearchQuery}
                  onChange={(e) => setLanguageSearchQuery(e.target.value)}
                  className="w-full pl-9 pr-4 py-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-xs text-slate-900 dark:text-zinc-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  autoFocus
                />
              </div>
              <button
                ...
                onClick={() => {
                  ...
                  setSuccessMsg("Editor set to Auto Language & Script Detection");
                }}
                className={`px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap cursor-pointer transition-all border ${
                  selectedLanguage === 'auto'
            ...
            {/* Languages Grid */}
            <div className="p-4 overflow-y-auto max-h-[50vh] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setIsLanguageModalOpen(false);
                        setSuccessMsg(`Switched language to ${lang.nativeName} (${lang.name})`);
                      }}
                      className={`p-3 rounded-2xl border text-left transition-all cursor-pointer flex flex-col justify-between group ${
                        isSelected
                          ...
                          : 'bg-white dark:bg-zinc-900/80 border-slate-200 dark:border-zinc-800 hover:border-indigo-300 dark:hover:border-indigo-700 hover:bg-slate-50 dark:hover:bg-zinc-800/50'
                      }`}
                    >
                      <div className="flex items-center justify-between gap-1 mb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="text-xl leading-none">{lang.flag}</span>
                          <span className="font-bold text-xs text-slate-900 dark:text-zinc-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
                            {lang.nativeName}
                          </span>
                        </div>
                        {lang.isRtl && (
                          <span className="text-[9px] font-bold px-1.5 py-0.5 bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 rounded-md border border-amber-200 dark:border-amber-900">
                            RTL
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-zinc-400">
                        <span>{lang.name}</span>
                        <span className="text-[10px] bg-slate-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-slate-600 dark:text-zinc-400 font-mono">
                          {lang.script}
                        </span>
                      </div>
                    </button>
            ...
            {/* Modal Footer */}
            <div className="p-4 border-t border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/50 flex items-center justify-between text-xs text-slate-500 dark:text-zinc-400">
              <div className="flex items-center gap-2">
                <span>Active: </span>
                <span className="font-semibold text-slate-800 dark:text-zinc-200">
                  {selectedLanguage === 'auto'
                    ? '🌐 Auto (Automatic Unicode Script Detection)'
                    : `${SUPPORTED_LANGUAGES_LIST.find(l => l.code === selectedLanguage)?.flag} ${SUPPORTED_LANGUAGES_LIST.find(l => l.code === selectedLanguage)?.nativeName} (${SUPPORTED_LANGUAGES_LIST.find(l => l.code === selectedLanguage)?.name})`}
                </span>
              </div>
              <button
                type="button"
                onClick={() => setIsLanguageModalOpen(false)}
                className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl cursor-pointer transition-colors shadow-sm"
              >
                Done
              </button>
            </div>
