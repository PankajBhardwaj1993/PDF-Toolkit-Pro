...
function AppContent() {
  ...
  if (pathSegments.length > 0) {
    if (pathSegments[0] === 'tools') {
      if (pathSegments[1]) {
        ...
        const matchedTool = allToolsList.find(t => t.id === routeId || t.id.replace(/_/g, '-') === routeId);
        if (matchedTool) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#0b0e14] text-slate-800 dark:text-slate-200 transition-colors duration-300">
      ...
      <main className="flex-grow">
        ...
        {is404 ? (
          ...
          <>
            {(() => {
              const currentTool = allToolsList.find(t => t.id === selectedToolId);
              if (currentTool) {
                return (
                  <SEO
                    title={currentTool.seoTitle || `${currentTool.name} | Free Online PDF Toolkit Pro`}
                    description={currentTool.seoDescription || currentTool.description}
                    canonical={`/tools/${selectedToolId}`}
                    keywords={currentTool.seoKeywords}
                  />
                );
              }
              return null;
          ...
          <>
            ...
            <SEO 
              title="PDF Toolkit Pro | Free Online PDF Editor, Converter & AI Workspace"
              description="The ultimate free online document workspace. Merge, compress, split, edit, sign, and convert PDF files directly in your browser securely. No signups, no limits, no watermarks."
              canonical="/"
              keywords={[
                'pdf toolkit pro',
                ...
                'pdf format converter docx xlsx',
              ...
              <div className="border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 bg-slate-50 dark:bg-zinc-900/20 text-center space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-zinc-400 leading-normal px-1">
                    You can donate any small amount of your choice, such as <span className="font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-md">₹10, ₹20, or ₹50</span>. Your small contribution helps keep our servers running and maintains the domain so we can continue serving you completely free tools!
                  </p>
                </div>
