...
// Persistent file-backed JSON state manager for the Express server
export class Database {
  private storageFilePath: string = path.join(process.cwd(), 'data_store.json');
  private _isDonationDisabled: boolean = false;

  get isDonationDisabled(): boolean {
    return this._isDonationDisabled;
  }

  set isDonationDisabled(val: boolean) {
    this._isDonationDisabled = val;
    this.saveToDisk();
  }
  ...
  blogPosts: BlogPost[] = [...initialBlogPosts];
  ...
  private loadFromDisk() {
    try {
      if (fs.existsSync(this.storageFilePath)) {
        ...
        if (typeof data.isDonationDisabled === 'boolean') {
          this.isDonationDisabled = data.isDonationDisabled;
        }
        if (Array.isArray(data.blogPosts)) {
          const loadedIds = new Set(data.blogPosts.map((b: BlogPost) => b.id));
          const missingDefaults = initialBlogPosts.filter(b => !loadedIds.has(b.id));
          this.blogPosts = [...missingDefaults, ...data.blogPosts];
        } else {
  ...
  saveToDisk() {
    try {
      const data = {
        isDonationDisabled: this.isDonationDisabled,
  ...
  setDonationDisabled(disabled: boolean) {
    this.isDonationDisabled = disabled;
    this.saveToDisk();
  }
  ...
  updateTicketStatus(ticketId: string, status: 'open' | 'pending' | 'resolved') {
    const ticket = this.supportTickets.find(t => t.id === ticketId);
    if (ticket) {
  ...
  deleteTicket(ticketId: string) {
    const beforeLen = this.supportTickets.length;
    this.supportTickets = this.supportTickets.filter(t => t.id !== ticketId);
    const wasDeleted = this.supportTickets.length < beforeLen;
    if (wasDeleted) {
      this.saveToDisk();
    }
    return wasDeleted;
  ...
  updateBlog(blogId: string, updatedFields: Partial<BlogPost>) {
    const index = this.blogPosts.findIndex(b => b.id === blogId);
    if (index !== -1) {
      this.blogPosts[index] = { ...this.blogPosts[index], ...updatedFields };
      this.saveToDisk();
