...
"${text.trim()}"`;
  ...
  // ==========================================
  // 2. SEARCH ENGINE & ADSENSE ENDPOINTS
  // ==========================================

  // Serve Google AdSense ads.txt (High priority direct endpoint)
  const serveAdsTxt = (req: express.Request, res: express.Response) => {
    res.status(200);
    ...
    if (fs.existsSync(publicAds)) {
      return res.send(fs.readFileSync(publicAds, 'utf-8').trim() + '\n');
    }
    if (fs.existsSync(distAds)) {
      return res.send(fs.readFileSync(distAds, 'utf-8').trim() + '\n');
    }
    return res.send('google.com, pub-6388269324760833, DIRECT, f08c47fec0942fa0\n');
  };

  app.get(['/ads.txt', '/ads.txt/'], serveAdsTxt);
