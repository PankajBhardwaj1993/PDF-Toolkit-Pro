...
function getToolSlug(tool: { id: string; name?: string; slug?: string }): string {
  ...
  return tool.id.toLowerCase().replace(/_/g, '-').replace(/[^a-z0-9-]/g, '');
}
...
"${text.trim()}"`;
  ...
  app.get('/robots.txt_old', (req, res) => {
    res.type('text/plain');
    res.send(`User-agent: *\nAllow: /\nSitemap: https://pdftoolkitpro.online/sitemap.xml`);
  });
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    app.get('*', async (req, res) => {
      try {
        ...
        let title = "PDF Toolkit Pro - Free Online PDF Editor & Converter";
        let desc = "The ultimate free online PDF toolkit. Merge, compress, edit, sign, and convert PDF files directly in your browser.";
        let url = `https://pdftoolkitpro.online${req.url}`;
        
        if (req.url.startsWith('/tools/')) {
          const toolId = req.url.split('/')[2];
          const tool = allToolsList.find(t => getToolSlug(t) === toolId || t.id === toolId || t.id.replace(/_/g, '-') === toolId);
          if (tool) {
            title = `${tool.name} - Free Online PDF Tool | PDF Toolkit Pro`;
            desc = tool.description;
          }
        } else if (req.url === '/dashboard') {
          title = "My Dashboard | PDF Toolkit Pro";
        } else if (req.url === '/pricing') {
          title = "Pricing Plans | PDF Toolkit Pro";
        }
        
        const ogImage = `https://pdftoolkitpro.online/og-image.svg?title=${encodeURIComponent(title)}`;
        
        // Replace existing title and meta
        template = template.replace(/<title>.*?<\/title>/, `<title>${title}</title>`);
        template = template.replace(/<meta name="description".*?>/, `<meta name="description" content="${desc}" />`);
        
        // Add OG tags and canonical if they don't exist in head (we can just inject before </head>)
        const seoTags = `
